...
class AssetController extends Controller
{
    ...
    {
        return $this->checkTextures(SyncHelper::getGgGear(), 'base_id', 'image', 'gear', 'Gear');
    }

    public function checkTexturesUnit()
    {
        return $this->checkTextures(SyncHelper::getGgChars(), 'base_id', 'image', 'characters', 'Chars');
    }

    public function checkTexturesShip()
    {
        return $this->checkTextures(SyncHelper::getGgShips(), 'base_id', 'image', 'ships', 'Ships');
    }

    public function checkTexturesAbility()
    {
        return $this->checkTextures(SyncHelper::getGgAbilities(), 'base_id', 'image', 'abilities', 'Abilities');
    }
    ...
    protected function findOrFixAsset(string $url, string $folder)
    {
        ...
        $filename = $folder . '/' . array_reverse(preg_split('~/~', $url, -1, PREG_SPLIT_NO_EMPTY))[0] . $this->texture_extension;
