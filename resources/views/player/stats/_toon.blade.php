...
<!-- <div class="stats-check-ignore">NOT IMPLEMENTED - was fehlt ggf?</div> -->
    ...
    @if($rosterToon['combatType'] == 'CHARACTER')
        ...
        @else
        <div class="text-danger"><i class="fa fa-bolt text-danger"></i> Der Einheit fehlen ({{ 6 - count($rosterToon['mods']) }} Mods.</div>
        @endif
    @endif
    ...
    @if($rosterToon['combatType'] == 'CHARACTER')
        @foreach($rosterToon['mods'] as $toonMod)
            ...
            @else
            <div class="text-danger"><i class="fa fa-bolt text-danger"></i> Slot {{$toonMod['slot']}}: Der Mod ist nur Stufe {{ $toonMod['level'] }}. Sofort auf Max bringen.</div>
            @endif
...
<!-- <div class="stats-check-ignore">NOT IMPLEMENTED - unvollständige Sets</div> -->
    @foreach($rosterToon['skills'] as $toonSkill)
        ...
        @elseif($toonSkill['tier'] == 8)
        <div class="stats-check-ok"><i class="fa fa-check text-success"></i> Fähigkeit <em>{{ $skillKeys[$toonSkill['id']] }}</em> ist auf Max ({{ $toonSkill['tier'] }}).@if($toonSkill['isZeta']) <span class="icon-zeta"></span>@endif</div>
        @elseif($toonSkill['tier'] == 7 && $rosterToon['combatType'] == 'CHARACTER')
        <div><i class="fa fa-info text-danger"></i> Fähigkeit <em>{{ $skillKeys[$toonSkill['id']] }}</em> ist auf Stufe {{ $toonSkill['tier'] }}.@if($toonSkill['isZeta']) Zeta fehlt noch.<span class="icon-zeta"></span>@endif</div>
            @if($toonSkill['isZeta'])
            ...
            @endforeach
            @endif
        @elseif($toonSkill['tier'] == 3 && false !== strpos($toonSkill['id'], 'hardwareskill'))
        ...
        <div class="text-danger"><i class="fa fa-bolt text-danger"></i> Fähigkeit <em>{{ $skillKeys[$toonSkill['id']] }}</em> ist nur Stufe {{ $toonSkill['tier'] }}. Sofort prüfen.@if($toonSkill['isZeta']) Ist zudem Zeta.<span class="icon-zeta"></span>@endif</div>
        @else
        <div class="text-danger"><i class="fa fa-times text-danger"></i> Fähigkeit <em>{{ $skillKeys[$toonSkill['id']] }}</em> ist nur Stufe {{ $toonSkill['tier'] }}.</div>
        @endif
    @endforeach
